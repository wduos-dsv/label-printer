...
function createWindow() {
  win = new BrowserWindow({
    frame: false,
    icon: path$n.join(process.env.VITE_PUBLIC, "icon.png"),
    height: 500,
    minHeight: 500,
    width: 700,
    minWidth: 700,
...
function getUnique5DigitCode() {
  ...
  const hashHex = crypto.createHash("sha256").update(timestampStr).digest("hex");
...
function genRecLabelZpl(uniqueCode, counter, manualCode, mode) {
  const barcodePayload = mode === "sequential" ? `REC${uniqueCode}${String(counter).padStart(3, "0")}ARQ` : `REC${manualCode}ARQ`;
  return `^XA~TA000~JSN^LT0^MNW^MTT^LH0,0^PR4,4~SD10^CI27^MMT^PW815^LL416^LS0^FT319,135^A0N,102,116^FH^CI28^FDLPN^FS^CI27^FO29,18^GB772,381,6^FS^FO658,340^GFA,305,688,16,:Z64::3FF8^FT242,324^A0N,46,46^FH^CI28^FD${barcodePayload}^FS^CI27^BY3,3,95^FT130,278^BCN,,N,N^FH^FD>:${barcodePayload}^FS^PQ1,0,1,Y^XZ`;
}
...
function genHuaweiExpZpl(destination, nf, packingList, thisLabel, totalLabels) {
